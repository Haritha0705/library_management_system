        ...
        <div className={"flex flex-col gap-4 items-center text-gray-800 py-16"} id={"speciality"}>
            ...
            <div className={"flex sm:justify-center gap-4 pt-5 pb-2 w-full overflow-scroll"}>
                {CategoryData.map((item,index)=>(
                    <Link onClick={()=>scrollTo(0,0)} className="flex flex-col items-center gap-2 flex-shrink-0 p-4 bg-blue-100 rounded-xl hover:scale-105 transition-transform duration-300 ease-in-out shadow-md hover:shadow-lg" key={index} to={`/books/${item.category}`}>
